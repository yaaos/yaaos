...
- **`bin/ci` IS THE TEST COMMAND. Run it before saying "done". No exceptions.** Don't run `pytest`, `pnpm test`, `ruff`, `tach`, `semgrep`, or `tsc` standalone for verification — they're each a slice of the full gate and skipping the others is how regressions slip in. The per-app scripts are the contract:
  ...
  - **Web code changed** (anything under `apps/web/` that isn't a `.md`) → `apps/web/bin/ci`. Includes `tsc --noEmit`, lint, vite build. The web-side semgrep scan is intentionally NOT in `apps/web/bin/ci` because the web-builder Docker stage is node-only — in CI it runs as a sibling RWX task (`web-security`) using the official `semgrep/semgrep` image. For local dev, run `cd apps/web && uv run --directory ../backend semgrep scan --config p/typescript --config p/react --config p/owasp-top-ten --error --metrics off --quiet src` (reuses the semgrep already pinned in the backend's uv venv — convenience shortcut, not a structural cross-app dep).
