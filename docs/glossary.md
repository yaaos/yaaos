...
| **Action** | A synchronous, deterministic control-plane stage executor (`domain/actions.Action` Protocol) — no agent invocation, no parking, SAVEPOINT-wrapped. Plugins contribute `Action`s via `register_action`; `ActionStage.action_id` keys into the registry. Result persists on `stage_executions.action_result`. |
