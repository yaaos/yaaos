...
| **Stale-claim guard** | `POST /api/v1/commands/{id}/events` returns `200 {"command_event_outcome": "stale_claim_dropped"}` when inbound `command_id` no longer matches `workspaces.current_command_id`. Agent abandons silently; the outcome is semantic, not HTTP-coded. |
