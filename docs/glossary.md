...
| Term | Meaning |
|---|---|
| **Org** | Tenant boundary. UUID PK + immutable unique `slug` used in `/orgs/{slug}/...` URLs. Multi-org from onward; users may belong to many. Every non-user data row is `org_id`-scoped. Soft-deleted via `archived_at`. |
...
| **Lesson** | Repo-scoped institutional memory: `{title, body, source_pr_url}`, 1000-char body cap. Surfaces in agent prompts; UI shows applied-lesson chips. Owned by `domain/lessons`. |
...
| **VCS plugin** | Plugin implementing `domain/vcs.VCSPlugin`. ships `github` only. At most one per org; install state stored on `orgs.vcs_plugin_id` + `orgs.vcs_settings`. Choice changed via Org Settings > VCS. |
...
| **Verified GitHub username** | `users.github_username`. Denorm written by the "Sign in with GitHub" login callback on every successful sign-in. Re-binding is "sign in with GitHub again." Never user-typed. |
...
| **BYOK** | Bring-your-own-key per `(org_id, provider)`. ships `anthropic` only. Plaintext lives only inbound on POST + outbound from `core/byok.get/.validate`; the DB stores Fernet ciphertext via `core/secrets`. |
...
| **Workflow** | Typed Pydantic data structure registered at startup. `name`, `version`, ordered `steps`, `entry_step_id`. Five ship (`pr_review_v1`, `incremental_review_v1`, `verify_fix_v1`, `stale_check_v1`, `answer_question_v1`). Owned by `core/workflow`. |
| **WorkflowCommand** | One unit of work within a workflow. Three categories: **Workspace** (issues AgentCommands; workflow parks in `awaiting_agent`), **Local** (runs inline in the worker), **HITL** (writes a pending-decision row, parks in `awaiting_human`). 13 ship in across `domain/reviewer/commands/` + `core/workspace/commands.py`. Engine in `core/workflow`. |
...
| **Workspace** | Persisted execution environment in `workspaces` table. `provider` discriminates `in_memory` vs `remote_agent`. Single-flight gated by `current_command_id` claim; `current_holder_workflow_id` links the holding workflow execution. |
...
| **`core/tasks`** | Thin abstraction wrapping taskiq + Redis. `@task(name)` registers a body; `enqueue(task_ref, args, *, session)` writes a `taskiq_enqueue` outbox row. Three task names: `workflow.start_step`, `workflow.handle_agent_event`, `workflow.route_workflow`. |
