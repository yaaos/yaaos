...
5. Every skill/review stage resolves its skill file at `.claude/skills/<skill_name>/SKILL.md` in the checkout (`stage.skill_name` from the pipeline definition) — the agent stats it before spawning claude and fails the stage loudly if absent.
...
3. Agent long-polls, runs the operation, reports via `POST /api/v1/commands/{id}/events`. Backend's `record_agent_event` validates the stale-claim guard, resolves `command_id → agent_commands.workflow_execution_id`, enqueues `HANDLE_AGENT_EVENT`.
