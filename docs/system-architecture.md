...
5. Every skill/review stage resolves its skill file at `.claude/skills/<skill_name>/SKILL.md` in the checkout (`stage.skill_name` from the pipeline definition) — the agent stats it before spawning claude and fails the stage loudly if absent. The shipped `pipeline-*` skills + agents reach a user's own repo via a "Download skills" link on the Pipelines settings page — a build-time zip (`apps/web/public/yaaos-pipeline-skills.zip`, entries repo-root-relative) unzipped straight into the repo root.
