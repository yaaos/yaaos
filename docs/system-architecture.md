...
- Claude Code CLI runs inside the WorkspaceAgent container (customer-deployed); spawned once per pipeline skill/review stage by the agent. The backend never execs the CLI in-process. It does make its own direct, single-shot LLM calls for text-only reasoning that doesn't need code access — `core/llm` (mechanics) + a calling domain module's `<module>/llm/` (prompts + schemas), e.g. `domain/pr_review`'s inbound-comment classifier.
...
3. The intake type verifies HMAC, parses payload, and (for opened/reopened/ready_for_review) resolves the `github:pr_opened` intake point against `domain/repos` trigger bindings for the repo. A bound repo gets a race-safe ticket + PR row insert, then `domain/pipelines.start_run` per binding — single transaction. An unbound repo is a 2xx no-op: no ticket, no run.
4. The run engine (`domain/pipelines`) drives the `ROUTE_RUN`/`START_STAGE`/`HANDLE_AGENT_EVENT` taskiq trio through the pipeline's stages (`skill`/`review`/`action`/`call`, flattened from the definition at run start). Skill and review stages dispatch a real coding-agent invocation and park the run on `pending_agent_command_id`; the terminal AgentEvent resumes it. Findings a review stage reports materialize immediately as durable `domain/findings` rows; an action stage (e.g. `github:create_pr`) posts them to GitHub via `vcs.post_finding` (named primitive args — no finding value object crosses the `vcs` boundary).
5. Every skill/review stage resolves its skill file at `.claude/skills/<skill_name>/SKILL.md` (or `.codex/skills/<skill_name>/SKILL.md` for codex) in the checkout (`stage.skill_name` from the pipeline definition) — the agent stats it before spawning the CLI and fails the stage loudly if absent. The `pipeline-*` skills + agents reach a user's own repo via per-agent "Download skills" buttons on the Pipelines settings page — each button hits `GET /api/coding-agents/{plugin_id}/skills-bundle` (authenticated, returns a ZIP built on the fly by the backend). The backend reads skill/agent sources from `YAAOS_SKILLS_SOURCE_DIR` (baked into the image at `/app/yaaos_skills`; dev default: repo `.claude/`) and calls `plugin.render_skill_bundle()` to produce a vendor-native layout before zipping.
...
### MCP context for coding-agent invocations

Coding-agent CLIs call hosted MCP servers (Linear, Notion) through a yaaos-owned proxy — authorization in one place, every JSON-RPC method writes an audit row.

Mechanics: a caller mints a bearer via `mcp_proxy.mint_token(review_id, org_id=...)` (raw `secrets.token_urlsafe(32)`, sha256 → `mcp_review_tokens`; `review_id` is a soft reference — any UUID scopes the token), builds an MCP payload (`integrations.known_providers()` filtered to enabled + non-failed), and threads `{token, base_url, servers}` into the coding-agent invocation so the CLI has MCP server config. The workspace writes `.mcp.json`; the CLI calls `mcp__linear__get_issue`; the proxy at `POST /api/mcp/{review_id}/linear` verifies the sha256 bearer, resolves `org_id`, enforces the write-tool allowlist, decrypts the org's access token, forwards to upstream, audits the call. `mcp_proxy.revoke_token` drops the token's rows; an hourly sweep also drops anything past its 2h TTL.

**Broken-creds surfacing.** Health-check flips `last_refresh_status`; audit row `mcp.<provider>.token_refresh_failed`; email to Owners (24h dedup); `GET /api/integrations/broken-summary` (cross-org; Owners + Admins only); red banner in the app shell; warning block in Coding Agents settings. `mcp_proxy.record_broken_creds`/`consume_broken_creds` track per-invocation broken-provider observations for a future review-output warning surface — no caller drains them today.
...
- **Run engine** (`domain/pipelines`) — data-defined `PipelineDefinition`s (skill/review/action/call stages) driven by the `ROUTE_RUN`/`START_STAGE`/`HANDLE_AGENT_EVENT` taskiq trio over `core/tasks` + `core/outbox`. Skill/review stages park the run on `pending_agent_command_id` and resume on terminal AgentEvent. See [`domain_pipelines.md`](../apps/backend/docs/domain_pipelines.md).
...
- **WorkspaceAgent** (`apps/agent/`) — customer-deployed Go binary; holds source code locally. Five HTTPS endpoints + one bidirectional WebSocket under `/api/v1/`. Agent identity on operational channels is bearer-derived — no `{agent_id}` path segment. Full protocol contract: [`docs/workspace-agent-protocol.md`](../docs/workspace-agent-protocol.md).
  ...
  - `POST /api/v1/commands/{id}/events` — AgentCommand events; terminal events resume the run.
...
1. GitHub webhook → `POST /api/intake/github` verifies HMAC, dedups via `X-Github-Delivery`, resolves the `github:pr_opened` intake point against `domain/repos` trigger bindings, creates a ticket, starts a run per binding via `domain/pipelines.start_run`. Records `traceparent` so downstream tasks share the trace.
2. `ROUTE_RUN` picks up; `START_STAGE` dispatches the first stage. A `skill`/`review` stage's dispatch enqueues an AgentCommand and parks the run on `pending_agent_command_id`; an `action` stage runs synchronously in-process.
3. Agent long-polls, runs the operation, reports via `POST /api/v1/commands/{id}/events`. Backend's `record_agent_event` validates the stale-claim guard, resolves `command_id → agent_commands.run_id`, enqueues `HANDLE_AGENT_EVENT`.
4. `HANDLE_AGENT_EVENT` clears the claim and resumes the run: validates the skill's structured output, applies the boundary decision (proceed / pause / send back), and enqueues `START_STAGE` for the next stage (or a system `cleanup-workspace` stage on terminal outcomes).
5. Every invoke-kind dispatch (`InvokeClaudeCode`, `InvokeCodex`) creates a `coding_agent_runs` row (`status=running`, started_at). The registered `CodingAgentRunSink` (in `core/coding_agent`) fires on the matching terminal `AgentEvent`, calls the plugin's `parse_result(outputs)`, writes `status`/`exit_code`/`tokens_in`/`tokens_out`/`duration_ms` onto the run row, and persists the rendered `ActivityLog` JSONB to the partitioned `coding_agent_activity` table (weekly partitions, ~4-week TTL). See [`apps/backend/docs/core_coding_agent.md`](../apps/backend/docs/core_coding_agent.md).
...
`outbox_entries`, `workspace_agents`, `bearer_tokens`, and the run engine's own tables — `pipelines`, `pipeline_runs`, `stage_executions`, `run_pauses`, `pipeline_findings`, `artifacts`, `repo_settings`, `repo_trigger_bindings`, `pr_comments` (see [`domain_pipelines.md`](../apps/backend/docs/domain_pipelines.md)). Existing tables extended: `tickets` (type, idempotency_key, payload, current_run_id, branch_name), `workspaces` (current_command_id, max_idle_seconds, owning_agent_id), `orgs` (registered_iam_arn, aws_region). Activity events are never persisted — they exist only in flight from WebSocket → `core/sse` → SSE → UI.
