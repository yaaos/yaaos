...
| `YAAOS_PUBLIC_ORIGIN` | **Required.** Full external origin (scheme+host[:port], e.g. `https://app.yaaos.cloud`). Boot fails if unset. Derives the link base (`yaaos_app_base_url`, for invitation/SAML/MCP URLs) and the agent-audience hostname (`yaaos_public_hostname` = its host[:port], validated against `X-Yaaos-Audience`; must equal `hostFromURL(YAAOS_BACKEND_URL)` on the agent side). |
...
The backend refuses to start when any required field (`DATABASE_URL`, `YAAOS_ENCRYPTION_KEY`, `REDIS_URL`, `YAAOS_PUBLIC_ORIGIN`) is unset. Optional secrets default to stub values in dev/test.
