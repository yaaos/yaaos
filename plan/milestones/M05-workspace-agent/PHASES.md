...
- [ ] **Local WorkflowCommands (3/5 real):** `CheckShouldReview` ✓ (slice 1: admission gate on draft/fork/skip-label/bot-author from ticket payload). `ArchiveStaleFindings` ✓ (slice 10: aggregate STALE state transition). `ResolveFinding` ✓ (slice 11: verify-fix verdict → RESOLVED_CONFIRMED). `PostFindings` + `PostReply` still stubs — `PostFindings` rides on the FindingDraft → RawFinding conversion + the new `admission.admit_raw_findings`; `PostReply` rides on `vcs.post_comment_reply` + thread lookup.
