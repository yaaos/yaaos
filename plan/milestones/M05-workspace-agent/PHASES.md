...
- [x] Reflection — Six axes walked. **Requirements**: 4/5 Workspace bodies + 5/5 Local bodies + lifecycle bodies + workflow engine routing + recovery integration + admission module + GitHub-post integration + e2e tests all shipped. The 5 Workspace reviewer command bodies (CodeReview, IncrementalReview, VerifyFix, StaleCheck, AnswerQuestion) still default to `Outcome.success()` rather than invoking real `coding_agent.<method>` calls — substrate (`_WorkspaceReviewCommand` base + `build_invocation`) is in place but the wiring is deferred-to-follow-on (Phase 6 Go subprocess body + tests-side stub_coding_agent integration are the next step). **Architecture**: workflow engine routes Workspace commands inline for in_memory, awaiting_agent for remote_agent; admission module owns the FindingDraft → RawFinding → vcs.Finding conversion pipeline; PostFindings/PostReply close their GitHub loops. Matches [architecture.md § Workflow execution model](architecture.md). **Testing**: 92 net new tests this phase across CheckShouldReview (11), lifecycle commands (8), workspace base contract (16), pr_review_v1 e2e with findings (2), all-workflows smoke (4), ArchiveStaleFindings (5), ResolveFinding (7), PostFindings (8 — including happy-path), PostReply (7 — including GitHub-post happy-path), admission (2), $ticket resolver (2), get_workspace (4), invocation builder (5), recovery policy (2), and more. **Observability**: structlog at every command body decision point; `admission.done` + `admission.drops` + `vcs_post.done` events mirror legacy queue.py emissions. **Security**: VCS post via registered plugin (auth from `vcs.get_installation_token`); no new attack surfaces. **Docs sync**: `domain_reviewer.md`, `core_workflow.md`, `core_workspace.md` all updated alongside each slice. Remaining gaps documented above (queue.py file deletion + review_jobs table drop) name their follow-on owner — the next M05 cleanup slice.
