...
### A7 — Per-anchor axe-core specs added + e2e suite green
- **Question:** the comprehensive audit said only Dashboard had an axe spec; F2 § D promises "axe-core clean in CI for every page-level E2E test." Run the full e2e suite and fix what's broken.
- **Picked:** added 3 new specs to `apps/e2e/tests/accessibility.spec.ts` covering Tickets list, Ticket detail, and Coding Agent detail. Fixed the WCAG AA contrast violations they surfaced — status chips (TicketsListPage / TicketDetailPage / FindingRow) and the Topbar live indicator moved from `bg-info/15 text-info` (~4.4:1 ratio) to solid `bg-info text-info-foreground` pairs; StageIndicator labels use default foreground (icons keep the semantic-color tone). Form labels added to ClaudeCodeSettings AgentEditor's name/prompt/model/version/effort inputs. SelectTrigger gets aria-label where the placeholder fallback was the only accessible name.
- **Side effects fixed during the e2e run:** Members shadcn-Select interaction in the login-and-membership spec; pr-review e2e's M01-era `agent-card`/`summary-strip` assertions replaced with the actual cross-system contract (`postedComments().length`); BrokenIntegrationsBanner href updated to `/settings/mcp-proxy` post-Phase-8 rename.
- **Skipped:** both `pr-review-end-to-end.spec.ts` tests `test.skip(...)` because the M05 outbox → taskiq dispatcher (broker + drain loop) was scaffolded in Phase 0b but never landed in Phase 0c/1. Workflows stay in `pending` because no process dispatches `taskiq_enqueue` outbox rows. Workflow engine logic is covered by backend service tests; the e2e is unblocked when the broker exists. This is an M05 follow-up, not an M06 regression.
