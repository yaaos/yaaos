...
- **STS endpoint is configurable via `YAAOS_STS_ENDPOINT_URL`** (default `https://sts.amazonaws.com/`). SigV4 binds the host into the signature, so the URL the agent signs against, the URL embedded in the envelope, and the URL the backend replays must all be identical — `resolveSTSEndpoint` is the single source for both signing and the envelope's `url` field. Dev/test compose points this at `http://mock-aws:4566/`.
