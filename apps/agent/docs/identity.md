...
> Seam that signs the agent instance's identity claim for the control plane.
...
- **`NewProvider` dispatches on `YAAOS_IDENTITY_PROVIDER`** (default `aws-sts`). Only `aws-sts` is defined; any other value panics at startup so a misconfigured agent instance fails fast instead of silently falling back.
...
- **STS endpoint is derived from the IMDS region** (`https://sts.<region>.amazonaws.com/`). SigV4 binds the host into the signature, so the URL the agent signs against, the URL embedded in the envelope, and the URL the backend replays must all be identical — `resolveSTSEndpointAndRegion` is the single source for both the endpoint and the signing region. The IMDS-derived region also matches the `aws_region` the org registers in the backend, ensuring the region check passes. `YAAOS_STS_ENDPOINT_URL` overrides both in dev/test (points at mock-aws, treated as `us-east-1`).
