...
- **Phase 8b follow-on slice 83 (this)** — supervisor wires the activity WebSocket end-to-end. New `Config.ActivityWSURL` (empty = disable, falls back to slice 76's HTTP-progress path) and `Config.ActivityBatchInterval` (defaults to 250ms). On `Run()` after identity exchange, when `ActivityWSURL` is set the supervisor dials `WSConn`, constructs an `activity.Conductor` with `WSConn.Send`, and spawns `RunInbound` so the backend's `subscribe`/`unsubscribe` messages drive the SubscriptionSet + WorkspaceMapping. The `progressForwarder` (slice 76) now routes through `Conductor.Publish` when WS is up — demand-pull: events for workspaces the backend hasn't subscribed to drop at the Conductor's gate. On WS dial failure the supervisor logs + continues with HTTP progress posts (correctness preserved; WS is an efficiency hop). On shutdown, Conductor.Stop / WSConn.Close run before pool reap. New `Conductor.IsSubscribed` accessor (replaces direct private-field reads) supports test/diagnostic introspection. 2 new tests cover the WS-enabled happy path (server pushes subscribe → Publish produces an activity_batch frame the server receives) and dial-failure-leaves-conductor-nil. Closes PHASES.md item 183.
- **Phase 8b follow-on slice 82** — `internal/activity.WSConn` wraps a single bidirectional WebSocket connection to the backend's `/api/v1/agents/{id}/activity` endpoint via [`github.com/coder/websocket@v1.8.13`](https://github.com/coder/websocket) (pinned to v1.8.13 because v1.8.14+ requires Go 1.23; the agent stays on Go 1.22). API: `Dial(ctx, wsURL, bearer)` injects the `Authorization: Bearer <token>` header; `Read(ctx)` blocks for one frame; `Send(frame)` writes one text frame with a 5s bounded internal timeout (so a wedged peer can't permanently block a flush goroutine); `Close()` ends with normal-closure status. Reconnect is the caller's job — if Read/Send error, dispose and re-dial. `RunInbound(ctx, conn, conductor)` is the read-loop helper that ties the WS transport to the slice-81 Conductor: pulls frames, dispatches each to `conductor.HandleInbound`, returns on ctx cancel or transport error. 4 unit tests cover dial-includes-bearer-header, send-reaches-server, read-delivers-server-frames, and the end-to-end RunInbound → Conductor → activity_batch back to the server. Tests use `httptest.NewServer` + the same `coder/websocket` server-side `Accept`. The supervisor's wiring becomes ~5 lines (dial, NewConductor with WSConn.Send, conductor.Start, go RunInbound, conductor.Publish from event sites) — that wiring slice is next.
- **Phase 8b follow-on slice 81** — `internal/activity.Conductor` composes the slice-78 + slice-80 primitives into a single consumer-facing API for the eventual WS-client: `HandleInbound(raw)` decodes one inbound frame and applies `subscribe`/`unsubscribe` to both the SubscriptionSet (filters outbound publishes) AND the WorkspaceMapping (translates workspace_id → workflow_execution_id for outbound frames). `Publish(workspaceID, ev)` forwards to the Batcher. On each flush tick the Conductor's adapter looks up the cached workflow_execution_id, encodes the activity_batch envelope, and calls a caller-supplied `SendFunc`. Missing mapping → drop the batch defensively (rather than send a malformed frame the backend would reject). Send errors are logged + dropped; transport-layer retry / reconnect is the caller's job. 7 unit tests cover subscribe-updates-both-stores, unsubscribe-removes-both, malformed-frame-errors, publish-flush-end-to-end, drop-when-unsubscribed, drop-when-mapping-missing (defensive), and send-error-is-not-fatal. The WS dial / read-loop / auth slice composes Conductor next — the WS code becomes a thin wrapper.
- **Phase 8b follow-on slice 80** — `internal/activity` protocol primitives. `DecodeInbound([]byte)` parses `subscribe`/`unsubscribe` WS messages (slice 79 payload shape: `workspace_id` + `workflow_execution_id`); errors on unknown `type`, missing `workspace_id`, or malformed JSON. `EncodeBatch(workflowExecutionID, []AgentEvent)` produces the outbound `activity_batch` JSON frame that the backend handler reads to address the right `activity:{workflow_execution_id}` SSE channel. `WorkspaceMapping` is a thread-safe `workspace_id → workflow_execution_id` cache the agent populates from inbound subscribe payloads so outbound batches don't need a backend round-trip. 9 unit tests cover decode happy paths, unknown-kind / missing-field / malformed-JSON error paths, encode round-trip, empty-events envelope, mapping set/get/remove, overwrite-on-duplicate, and 100-goroutine concurrent access. WS-client wiring (connect + auth + protocol-loop integration) is the next slice; these primitives are the building blocks.
- **Phase 8b follow-on slice 78** — `internal/activity` package: producer-side primitives for the demand-pull WebSocket activity-batching path (PHASES.md item 183). `SubscriptionSet` is a thread-safe set of subscription keys (workspace ID per the wire spec) mirroring the backend's `SubscriberRegistry` — `subscribe`/`unsubscribe` WS messages drive Add/Remove. `Batcher` accepts `Publish(key, AgentEvent)` calls, drops events whose key isn't subscribed (saves memory for activity the UI won't consume), buffers the rest per key, and flushes one batch per key on each interval tick (250ms in prod, parameterized for tests). Empty ticks are silent — no useless WS frames. `Stop()` drains the buffer synchronously so shutdown loses nothing. 8 unit tests cover add/contains/remove, double-add idempotency, 100-goroutine concurrent access (race detector), drop-when-unsubscribed, interval flush, per-key partitioning, drain-on-Stop, no-flush-on-empty-tick, and unsubscribe-mid-flight filtering. WS-client integration (connect, auth, subscribe/unsubscribe handler, sending `activity_batch` frames) lands in a future slice; for now `FlushFunc` is the integration seam.
- **Phase 6 follow-on slice 76** — live progress-event streaming end-to-end. New `internal/workspace.Emitter` interface + ctx plumbing; `workspace.Run` installs an `encoderEmitter` that writes `kind=progress` AgentEvents to the same IPC encoder the dispatch loop uses for the terminal event (`ipc.Encoder` is goroutine-safe so concurrent progress writes from the handler interleave correctly with the dispatcher's final write). `RealHandler.InvokeClaudeCode` pulls the emitter via `EmitterFromContext(ctx)` and wires `RunStreaming.OnStdoutLine` to push each Claude Code stream-json line as a progress event AND accumulate it locally so the terminal event still carries the full stdout. Supervisor side: `WorkspaceRunner.Send` now takes an `onProgress func(AgentEvent)` callback; runners read events in a loop, forwarding each progress event to the callback synchronously and returning on the first `kind=completed_*` event. `Pool.Dispatch` threads the callback through; `supervisor.routeCommand` wires it to `client.PostCommandEvent` so each progress event posts upstream as its own `EventProgress` AgentEvent (the backend's `record_agent_event` handles them without triggering workflow-engine resumption — only `completed_*` events resume). 5 new tests across two packages cover the emitter ctx plumbing, the encoder-emitter wire format, concurrent Progress safety (100 goroutines), multi-event dispatch end-to-end through `workspace.Run`, and the Pool → onProgress forwarder. Activity-batching over a dedicated WebSocket (PHASES.md item 183) is a separate follow-on; for now each progress event is its own HTTP POST.
