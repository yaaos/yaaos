...
- **Capability seams in `RealHandlerConfig`:** `CloneFunc` (git clone) and `RunFunc` (Claude Code subprocess; also used by `RunCodex` for the Codex subprocess). Both default to their production implementations; tests inject fakes. See [patterns.md § Testing](patterns.md#testing) for the fake-at-seam convention.
...
- **Progress events do not resume the backend's run engine** — only `completed_*` events do. `RealHandler.RunClaude` and `RealHandler.RunCodex` both stream stdout lines as `kind=progress` while accumulating the final result.
...
- **`RunFunc`** — function type for subprocess dispatch (`func(context.Context, RunStreamingOptions) (*RunStreamingResult, error)`); `RealHandlerConfig` field shared by `RunClaude` and `RunCodex`; production default is `RunStreaming`. Tests inject a fake to avoid spawning a real Claude or Codex binary.
