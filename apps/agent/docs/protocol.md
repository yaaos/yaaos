...
- **Flat wire shape.** The backend sends all command fields as a flat JSON object with `kind` embedded. Each concrete struct has `CommandHeader` embedded so `kind`, `command_id`, `workspace_id`, and `traceparent` are always present.
