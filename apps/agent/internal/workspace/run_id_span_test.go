// Tests that RunID on a CommandHeader is stamped as run_id
...
const testRunIDWorkspace = "aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee"
...
func buildCleanupCmd(workspaceID, commandID, runID string) command.WorkspaceCommand {
...
// TestExecuteCommand_RunID_PresentOnSpan verifies that when a command
// carries a non-empty RunID, the workspace.handle.<kind> span
// carries a `run_id` attribute equal to that value.
func TestExecuteCommand_RunID_PresentOnSpan(t *testing.T) {
	exp := tracing.Init(true)
	defer exp.Reset()
	t.Cleanup(func() { tracing.Init(false) })

	cmd := buildCleanupCmd("ws-wf-exec-1", "cmd-wf-exec-1", testRunIDWorkspace)
	...
	if gotRunID != testRunIDWorkspace {
		t.Errorf("run_id attr: want %q, got %q", testRunIDWorkspace, gotRunID)
	}
}

// TestExecuteCommand_RunID_AbsentWhenEmpty verifies that when the
// CommandHeader has an empty RunID, the workspace.handle.<kind>
// span does NOT carry a `run_id` attribute.
func TestExecuteCommand_RunID_AbsentWhenEmpty(t *testing.T) {
	exp := tracing.Init(true)
	defer exp.Reset()
	t.Cleanup(func() { tracing.Init(false) })

	// RunID is empty string (zero value).
