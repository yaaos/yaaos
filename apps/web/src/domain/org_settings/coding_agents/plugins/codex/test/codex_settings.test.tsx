/**
 * Component/MSW tests for CodexSettings — OpenAI API key card.
 */
...
describe("CodexSettings (MSW)", () => {
  ...
  it("renders the OpenAI key card unconditionally, regardless of install settings", async () => {
    server.use(
      http.get("/api/coding-agents", () =>
        HttpResponse.json([{ ...INSTALL, settings: { unrelated: "value" } }]),
      ),
    );
    render(wrap(<CodexSettings pluginId="codex" />));
    await waitFor(() => expect(screen.getByTestId("codex-key-not-set")).toBeInTheDocument());
