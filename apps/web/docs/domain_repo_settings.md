...
- **Protected code** (`ProtectedCodeSection.tsx`) — a deny/allow `RadioGroup`; switching modes opens an `AlertDialog` ("This inverts what's protected.") before the draft actually changes, since inversion is a non-obvious side effect. Path sets render as compact rows (name label + Edit + Delete buttons). Edit (and Add) opens a right-anchored `Sheet` containing a name `Input` (required, ≤100 chars), a globs `Textarea` (one glob per line), and an owners `UserMultiSelect`; Save is disabled while the name is empty. Delete opens an `AlertDialog` ("Delete \<name>?" / "This can't be undone." / red Delete) before removing the row. New rows mint their id via `crypto.randomUUID()` before Save (`ProtectedPathSet.id` is required on the wire).
...
The wire `RepoConfigView` (`core/api/public/queries`) is what the backend returns. The settings form works over a local draft (`types.ts`): path-set globs flattened to one-per-line textarea text, path-set `name` carried through as-is, and the loosely-typed `auto_approve_conditions` dict narrowed to the four known boolean fields. `configToDraft` / `draftToSpec` convert at the form's boundary. Trigger bindings are excluded from this draft — they're separate, immediately-committed calls.
...
- `test/repo-settings.test.tsx` — component/MSW: protected-code mode-switch confirm flow (dialog blocks the flip until confirmed, cancel leaves the mode unchanged); trigger-form schedule-field validation (schedule fields appear only for a `schedule`-kind intake point, and the submit button stays disabled until name/cron/notify are filled); path-set rows render name + Edit/Delete buttons; Edit opens the Sheet and saving updates the row name in the draft; empty name disables Sheet Save; Delete confirm (cancel leaves, confirm removes).
