...
- User details page Connections section: `connections-section` (container, present only when ≥1 provider is registered), `connection-row-${provider_id}` (per-provider card), `connection-connect-${provider_id}` (Connect button — shown when `status != "connected"`), `connection-connect-err-${provider_id}` (inline red error text — shown when the device-auth start mutation fails), `connection-disconnect-${provider_id}` (Disconnect button — shown when `status == "connected"`), `device-auth-dialog` (device-auth Dialog), `device-auth-verification-url` (URL display), `device-auth-user-code` (one-time code display), `disconnect-confirm` (disconnect confirm Dialog), `disconnect-confirm-action` (red Confirm/Disconnect button). See [domain_user.md](domain_user.md).
...
Backend emits ISO-8601 UTC (`Z`). FE renders in browser local timezone via `apps/web/src/shared/utils/public/ago.ts` and `duration.ts`:

- `ago(ts)` — relative duration (`"12s ago"`).
- `duration(start, end)` — compact elapsed-time span (`"42s"` / `"3m 12s"` / `"1h 04m"`); `end === null` computes elapsed against `Date.now()` at render time (no timer — SSE-driven invalidations re-render the caller). Used for run/stage timing on the ticket page's Runs tab.
