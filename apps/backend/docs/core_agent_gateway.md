...
- **Agent-event consumer registry (coexistence bridge).** `register_agent_event_consumer(task_ref)` appends to a module-private list — list-shaped, like the web/worker shutdown registries in `app.core.shutdown_registry`. `enqueue_agent_event(*, workflow_execution_id, agent_command_id, outcome_label, outputs, traceparent, session)` fans the identical args dict out to every registered consumer; `record_agent_event` calls it on every terminal `AgentEvent` instead of importing one engine's task directly. `core/workspace.failsafe_agent_loss` (agent-loss failsafe) also calls it directly — it synthesizes a terminal failure without an inbound `AgentEvent`, so it needs the same fan-out both engines rely on. Two consumers are registered today: `core/workflow.handle_agent_event` (registers itself at `core/workflow` import) and `domain/pipelines.handle_agent_event` (registers itself at `domain/pipelines` import). `workflow_execution_id` means "an id this engine owns" to each — an old-engine `WorkflowExecutionRow` id reaching the pipelines consumer, or a `pipeline_runs.id` reaching the old engine's consumer, resolves to `None` on that engine's own lookup and the consumer no-ops (logged, not raised). `agent_gateway` never imports either engine — both register themselves. The old engine's registration is removed when `core/workflow` is deleted.
...
`test/test_service.py` covers: heartbeat reports unknown workspaces; a terminal event enqueues every task registered via `register_agent_event_consumer`; progress events publish to the workspace-activity channel but do NOT enqueue; stale `command_id` raises `StaleClaimError`; `has_any_reachable_agent` respects the 90s cutoff and excludes `lifecycle='shutdown'` agents (defense in depth — `mark_agent_shutdown_complete` already pins `state='offline'`).

`domain/pipelines/test/test_event_consumer_seam_service.py` covers the coexistence bridge end-to-end: one `record_agent_event` call reaches both `core/workflow.handle_agent_event` and `domain/pipelines.handle_agent_event`; each no-ops on a `workflow_execution_id` it doesn't own; the old engine's flow still resumes through the shared registry.
