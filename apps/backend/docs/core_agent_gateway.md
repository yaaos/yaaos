...
- **Does not own:** workspace lifecycle (owned by [`core/workspace`](core_workspace.md)); workflow routing (owned by [`core/workflow`](core_workflow.md)); `coding_agent_runs` table (owned by [`domain/coding_agent`](domain_coding_agent.md)). Never imports `core/workspace` directly — workspace-state access goes through the registered sink.
...
- **Stale-claim guard (410)** — events whose `command_id` has no matching `agent_commands` row are rejected with `StaleClaimError`; the endpoint maps it to 410 Gone. The row may have been retired by an earlier terminal event.
...
- **`AgentRunSink` IoC seam** — `domain/coding_agent` implements the Protocol and registers `CodingAgentRunSinkImpl` at import time (`coding_agent/__init__.py`). `record_agent_event` calls the registered sink on every terminal AgentEvent; the sink no-ops for non-`InvokeClaudeCode` kinds. `get_run_sink()` returns `None` when the domain module hasn't been loaded — graceful degradation in minimal test configs. See `app/core/agent_gateway/run_sink.py`.
