...
- **AgentRef** — `agent_id` + `instance_id`; returned by `pick_agent_for_org` (least-loaded reachable pod). Queue operations (enqueue/claim/queue_depth) all key on `agent_id` (= `WorkspaceAgentRow.id`).
