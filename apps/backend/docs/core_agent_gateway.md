...
- **Cross-pod subscribe routing uses Redis pub/sub.** `_senders` stays process-local (the WS send callable only exists on the pod where the WS terminates). `track(wfx_id, workspace_id, agent_id)` writes to three Redis keys (`workflow_subscribers:{wfx_id}` ZSET, `workflow_route:{wfx_id}` HASH, `agent_routes:{agent_id}` SET) then publishes a typed `AgentWsControlMessage` on `agent_ws_control:{agent_id}`. The pod owning the agent's WS has a background task subscribed to that channel; it receives the message and forwards it to the live sender. `untrack` removes the ZSET member and, when ZCARD drops to zero, publishes `unsubscribe`.
- **`SubscriberReconciler` is the at-most-once safety net.** Runs every 5 s on each web pod. For each agent with a registered sender, reads `agent_routes:{agent_id}` and per wfx_id checks ZCARD truth: ZCARD ≥ 1 and agent not streaming → re-publish subscribe; ZCARD = 0 and agent is streaming → publish unsubscribe. Launched via `RouteSpec.on_startup` and stopped via `shutdown()` on SIGTERM.
- **`subscriber_sweeper` GCs stale ZSET entries.** A `@scheduled("subscriber_sweeper", "* * * * *")` task runs `ZREMRANGEBYSCORE` on every `workflow_subscribers:*` key every minute, removing members whose score (unix timestamp) is older than 60 s. Covers crashed SSE pods that never called `untrack`.
...
`test/test_subscriber_sweeper_scheduled_service.py` covers: `subscriber_sweeper` is registered with the taskiq broker; sweeper body removes ZSET members older than the stale threshold and leaves fresh members untouched.
