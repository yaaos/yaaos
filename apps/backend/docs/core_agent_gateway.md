...
- **Host allowlist override** — `YAAOS_STS_HOST_OVERRIDE` allows an additional STS host (e.g. `mock-aws:4566`) only when `APP_MODE` is non-production. The process refuses to boot with both `APP_MODE=production` and the override set.
