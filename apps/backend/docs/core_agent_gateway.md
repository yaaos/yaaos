...
- **Agent-event consumer registry.** `register_agent_event_consumer(task_ref)` appends to a module-private list — list-shaped, like the web/worker shutdown registries in `app.core.shutdown_registry`, so `agent_gateway` never imports the consumer's module directly (both register themselves at their own import time). `enqueue_agent_event(*, workflow_execution_id, agent_command_id, outcome_label, outputs, traceparent, session)` fans the identical args dict out to every registered consumer; `record_agent_event` calls it on every terminal `AgentEvent`. `core/workspace.failsafe_agent_loss` (agent-loss failsafe) also calls it directly — it synthesizes a terminal failure without an inbound `AgentEvent` and needs the same fan-out. One consumer is registered today: `domain/pipelines.handle_agent_event` (registers itself at `domain/pipelines` import). A `workflow_execution_id` that isn't a `pipeline_runs.id` resolves to `None` on the consumer's own lookup and it no-ops (logged, not raised) — the guard that keeps a stray/foreign id from ever raising.
...
`domain/pipelines/test/test_event_consumer_seam_service.py` covers the consumer registry's foreign-id guard: a terminal event stamped with a `workflow_execution_id` that isn't any `pipeline_runs.id` is a no-op that leaves a parked run untouched; the parked run's own terminal event still resumes it normally.
