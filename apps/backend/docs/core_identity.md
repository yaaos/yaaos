...
- **Public interface** — all functions are flat re-exports from `app.core.identity`. Session lifecycle functions carry explicit prefixes to avoid collision: `mint_session`, `lookup_session`, `revoke_session`, `revoke_all_sessions_for_user`, `rotate_session`, `touch_session`, `cleanup_expired_sessions`. TOTP functions: `enroll_totp`, `verify_totp`. User lookup: `find_user_ids_by_github_username(github_username: str, *, session: AsyncSession) -> list[UUID]` — case-insensitive scan of `users.github_username`; returns all matching user IDs (typically zero or one, but the schema allows more if a username was ever shared). Run attribution: `resolve_github_attribution(github_username, *, org_id, ticket_id, session) -> UUID | None` — resolves the username to the single active org member it belongs to; used at PR-intake and PR-comment time to stamp `pipeline_runs.triggered_by_user_id`. On any failure (blank username, no matching user, collision, non-member) it writes a `ticket.attribution_failed` audit row (payload: `github_username` + `reason`) and returns `None` — an unresolvable PR author is operator-visible, never silent. Covered by `test/test_github_attribution_service.py` (`@pytest.mark.service`). Callers never import `repository`, `sessions`, or `totp` submodules directly — use the flat names from the package root.
