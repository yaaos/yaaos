...
| `USER_SCOPED` | yes (via route dep) | **no** | n/a | `/api/account/*`, `/api/auth/me`, `/api/notifications`, `/api/orgs/mine`, `POST /api/orgs` |
...
Unclassified `/api/*` paths fall through as `PUBLIC` — legacy M01-era routers (intake, parts of `/api/reviewer` not yet backfilled) keep working. The post-response guard catches any 2xx response that escapes without `route_security_resolved` being set.
