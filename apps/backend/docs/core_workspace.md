...
Carries `id` plus three methods: `info()` returns `WorkspaceInfo`; `run_coding_agent_cli(argv, *, env=None, stdin=None, timeout_seconds=None, on_stream_line=None)` returns `CodingAgentCliResult`; `read_text(path)` returns the workspace-relative file content or `None`. No `working_dir` exposed. Callers hand `argv` + `env` + `stdin` to the workspace; the workspace forwards to the provider, which decides where and how (cwd, container, sandbox). `read_text` is the narrow file-read hook the incremental-review anchor re-resolver uses — providers implement it with path-traversal protection. Subprocess timeout + process-group kill are the provider's responsibility.
