...
- **Terminal-failure synthesis on agent loss.** For each expired workspace with a non-null `current_command_id` + `current_holder_workflow_id`, `failsafe_agent_loss` enqueues a `workflow.handle_agent_event` outbox row with `outcome_label="failure"` so the owning WorkflowExecution resumes (fails the step) rather than waiting forever in `AWAITING_AGENT`.
...
- `workspaces` — `(id, org_id, owning_agent_id, provider_id, spec jsonb, plugin_state jsonb, status, current_command_id, current_holder_workflow_id, max_idle_seconds, created_at, activated_at, expires_at, destroyed_at, destroy_attempts, last_destroy_attempt_at, last_destroy_error)`. `owning_agent_id` is the owning `workspace_agents.id` (soft FK; nullable; set at lean row creation for remote-agent rows). `plugin_state` is NULL for lean-created rows (legacy in-process path writes it). Indexes: `(status, expires_at)`, `(org_id, created_at)`, `current_holder_workflow_id`, `org_id`.
