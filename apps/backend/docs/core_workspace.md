...
- **Lean workspace row creation.** When the remote agent reports the first `created` or `ready` workspace event and no `workspaces` row exists, the sink inserts the row with `status='active'`, `owning_agent_id` from the reporting bearer, and `org_id`/`spec` resolved from the originating `agent_commands` row (via `get_command_org_and_payload`). The `workspace_id` is minted up front in `ProvisionWorkspace.dispatch`; the row materialises only once an agent owns it. The row never passes through `creating`.
