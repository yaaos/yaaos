...
`app/core/events/test/test_pubsub.py` covers the publish/subscribe contract — filter matching, queue overflow drop, unregistration on consumer exit. Tests use `async for ... return` inside a task coroutine; the async generator self-cleans on consumer exit so no explicit reset is needed. `app/core/events/test/test_subscribe_self_cleans.py` explicitly asserts subscriber count returns to baseline after consumer exit via both `async for ... return` and `async with aclosing(...)`. See [patterns.md § Subscription self-cleanup](patterns.md). `shutdown()` smoke tests live in `app/core/events/test/test_shutdown.py`. The SSE endpoint is exercised end-to-end against `TestClient`.
