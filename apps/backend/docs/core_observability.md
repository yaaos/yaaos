...
`core/intake/web.post_intake` records `current_traceparent()` when a webhook arrives and passes it into `core/workflow`; downstream tasks restore it. This is what gives one `trace_id` across webhook → terminal outcome.
...
`workflow.command.{kind}` is the per-command child span opened for every command category. It is opened via `with_remote_parent_span(wfx.otel_trace_context)` making it a direct child of `workflow.run.<name>` regardless of the current taskiq dequeue context. For Workspace commands it wraps the `dispatch()` call in `_start_step_impl`; for Local/HITL it wraps `execute()` inside `_safe_execute`. Both receive `StatusCode.ERROR` on any failure outcome; only the exception path adds an `exception` span event. `task:workflow.*` spans (taskiq auto-spans) also land in the workflow trace: `enqueue` auto-fills `TaskMetadata.traceparent` from the current span, and `TaskSpanMiddleware.pre_execute` uses it as the parent context for `task:<name>` spans — see [`core_tasks.md § Traceparent propagation`](core_tasks.md). See [`patterns.md § Exception visibility`](patterns.md#exception-visibility--record_exception-rule) for the rule and canonical shape.
