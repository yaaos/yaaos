...
`core/intake/web.post_intake` records `current_traceparent()` when a webhook arrives and passes it downstream; the run engine's task bodies restore it. This is what gives one `trace_id` across webhook → terminal outcome.

**Standard dims on every span + log** — `YaaosDimensionsSpanProcessor` is registered on the `TracerProvider` during `_configure_otel`. Its `on_start` reads the auth contextvars (`org_id_var`, `user_id_var`, `actor_kind_var`, `run_id_var`, `command_id_var` from `core/auth/context`) and stamps the non-None values as `yaaos.org_id`, `yaaos.user_id`, `yaaos.actor_kind`, `yaaos.run_id`, `yaaos.command_id` on every new span. OTel attributes do not inherit to child spans — `on_start` is the only mechanism that makes dims universal without per-span code. The `_YaaosLogDimsFilter` (stdlib `logging.Filter` added to the root logger by `configure()`) does the same for log records: it sets matching `LogRecord` attributes (`yaaos_org_id`, `yaaos_user_id`, etc.) from contextvars, which the OTel `LoggingHandler._get_attributes` then maps to queryable log attributes in Dash0. Both filters only stamp when the var is set — background spans carry org+actor but no `user_id`; `run_id_var`/`command_id_var` have no production setter today, so no span currently carries `yaaos.run_id`/`yaaos.command_id`.
...
`domain/pipelines`' run engine (`ROUTE_RUN`/`START_STAGE`/`HANDLE_AGENT_EVENT`) emits no manual spans today — it relies entirely on auto-instrumentation (`task:pipelines.*` taskiq spans + SQLAlchemy spans). `enqueue` auto-fills `TaskMetadata.traceparent` from the current span, and `TaskSpanMiddleware.pre_execute` uses it as the parent context for `task:<name>` spans, so a run's task spans still chain into one trace — see [`core_tasks.md § Traceparent propagation`](core_tasks.md). See [`patterns.md § Exception visibility`](patterns.md#exception-visibility--record_exception-rule) for the rule and canonical shape.
