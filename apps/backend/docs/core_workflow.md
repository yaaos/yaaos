...
- **Owns:** `Workflow`, `Step`, `WorkflowCommand` types, engine state machine, three taskiq task bodies (`start_step`, `handle_agent_event`, `route_workflow`), `workflow_executions` + `pending_human_decisions` tables, recovery-policy registry (`app/core/workflow/recovery.py`), terminal-hook registry (`app/core/workflow/terminal_hooks.py`), start-hook registry (`app/core/workflow/start_hooks.py`).
...
- **Start-hook registry** (`app/core/workflow/start_hooks.py`) — mirrors the terminal-hook registry for the bootstrap transition (when `completed_step_id is None` in `_route_workflow_impl`). Invoked after `wfx.state = RUNNING` and `_publish_state_changed` but before `_enqueue_start_step`, inside the same bootstrap-commit transaction. Hook signature (keyword-only): `workflow_execution_id: UUID, workflow_name: str, ticket_id: UUID, org_id: UUID, session: AsyncSession`. Registered via `register_start_hook(fn)` (idempotent); retrieved via `get_start_hooks()`; cleared for test isolation via `_clear_start_hooks_for_tests()` (direct submodule import). A raising hook rolls back the entire bootstrap write. Currently: the reviewer domain registers one hook (`domain/reviewer/start_hook.py`) that flips the owning ticket from `pending` to `running` atomically with the RUNNING state write.
...
| `workflow.command.<Kind>` | `workflow.run.<name>` — opened via `with_remote_parent_span(wfx.otel_trace_context)` so the span is a direct child of the run span regardless of which taskiq worker dequeued the task | `service.py` `_start_step_impl` (Workspace) and `_safe_execute` (Local/HITL) — same name for all categories | `command.kind`, `command.category`, `workflow.step_id`, `workflow.attempt`; `StatusCode.ERROR` on raise or `Outcome.failure`; for Workspace commands the span closes before `dispatch()` returns so `agent_command.dispatch.<Kind>` is a child of it |
...
`test/test_start_hooks_service.py` — start hook is invoked once with expected primitive kwargs during bootstrap; no-hook bootstrap succeeds without error.
