...
- **`workflow_state_changed` SSE on every transition** — every `wfx.state = …` assignment is immediately followed by `_publish_state_changed(s, wfx)`, which calls `publish_general_after_commit` with `GeneralEventKind.WORKFLOW_STATE_CHANGED` and payload `{ticket_id, workflow_execution_id, state}`. The SPA's Ticket page consumes this to invalidate its run-view cache without polling. Adding a new state assignment without the companion publish is the documented regression — covered by `test/test_state_changed_sse_service.py`.
