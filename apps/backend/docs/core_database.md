...
**`set_db_session_for_tests`** (exported from `app.core.database`) — context manager that routes every `async with session()` call to the fixture-bound `AsyncSession` for the duration of the block. The `db_session` fixture in `conftest.py` wraps the test body with `with set_db_session_for_tests(async_session):`. The `restart_savepoint` listener turns production `await s.commit()` into a SAVEPOINT release; teardown rolls back the outer transaction.
