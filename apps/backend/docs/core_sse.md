...
`test/test_shutdown_service.py` — `shutdown()` on a live general stream and a live workspace-activity stream each emit the final frame and raise `StopAsyncIteration`; idle shutdown doesn't raise; reset clears a stale set event.
