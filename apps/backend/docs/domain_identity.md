...
HTTP routes for login + callback + logout live in [`domain/sessions`](domain_sessions.md) (`/api/auth/*`). Account-management routes live under `/api/account/*` in `account_web.py`: emails, plus `GET/PATCH /api/account/me` for the user profile. All `/api/account/*` routes are `RouteSecurity.USER_SCOPED` — session via `_require_account()` (which delegates to `domain/sessions.require_session`), no `X-Org-Slug` required. The Phase 3 skeleton wires an `on_startup` hook that spawns the periodic cleanup loop.
