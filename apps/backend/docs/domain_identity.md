...
HTTP routes for login + callback + logout live in [`domain/sessions`](domain_sessions.md) (`/api/auth/*`). Account-management routes live under `/api/account/*` in `account_web.py`: emails, plus `GET/PATCH /api/account/me` for the user profile. The Phase 3 skeleton wires an `on_startup` hook that spawns the periodic cleanup loop.

`users.github_username` is a denorm written by the login orchestrator on every GitHub sign-in (see Login orchestrator below). The Account > Details page only displays it and offers a Clear button (`clear_github_username: true` on `PATCH /api/account/me`). Re-binding to a different GitHub account is "sign in with GitHub again."
