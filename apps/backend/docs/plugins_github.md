...
5. `issue_comment.created`/`pull_request_review_comment.created` (bot comments filtered first): a ticket whose `current_run_id` is set (ever ran under pipelines) routes the WHOLE comment — `@yaaos` grammar and free text alike — to `domain/pr_review.handle_pr_comment`. A ticket that has never had a pipeline run keeps today's grammar directly (`core/intake.parse_yaaos_command` → `cancel`/`re-review` against `domain/reviewer.cancel_workflows_for_ticket`/`start_pr_review` — coexistence fallback, removed once the old engine is deleted). See [domain_pr_review.md](domain_pr_review.md).
