...
- **Seven remote-dispatch methods.** Legacy path: `build_review_invocation`, `parse_review_output`, `review_preflight_steps`, `parse_usage`, `render_activity`. New path: `build_invocation`, `parse_result`. The run sink calls `parse_result` exclusively; `parse_usage` / `render_activity` remain on the Protocol for plugins that need to call them independently. Adding a mode requires a Protocol change.
