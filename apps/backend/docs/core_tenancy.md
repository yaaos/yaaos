...
- **OrgFullView** — extended org projection including all feature columns (`session_timeout_override`, `registered_iam_arn`, `aws_region`, `workspace_max_count`, `vcs_plugin_id`, `vcs_settings`). Returned by `get_org_full`, `get_org_full_by_slug`, `update_org_fields`, etc. Used by any module that needs to read org feature columns without importing `OrgRow` — including `domain/orgs`, `core/intake`, `core/agent_gateway` (for the `workspace_max_count` cap delivered via ConfigUpdate), and `plugins/github`. `workspace_max_count` is NOT NULL with server default 4 + CHECK 1..50.
...
Test teardown that hard-deletes an org row uses `delete_org` from `app.testing.e2e_setup` — there is no production org-deletion flow.
