...
- **OrgFullView** — extended org projection including all feature columns (`session_timeout_override`, `workspace_provider`, `registered_iam_arn`, `aws_region`, `vcs_plugin_id`, `vcs_settings`). Returned by `get_org_full`, `get_org_full_by_slug`, `update_org_fields`, etc. Used by `domain/orgs` when it needs to read or update org columns without importing `OrgRow`.
