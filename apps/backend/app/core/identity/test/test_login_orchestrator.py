...
async def test_existing_identity_returns_existing_user(db_session) -> None:
    user = await insert_user(db_session, display_name="Old")
    await add_email(db_session, user_id=user.id, email="user@example.com", verified=True)
    await add_oauth_identity(db_session, user_id=user.id, provider="github", external_subject="42")
...
async def test_email_match_without_identity_autolinks(db_session) -> None:
    ...
    await add_email(db_session, user_id=user.id, email="user@example.com", verified=True)
    await add_oauth_identity(db_session, user_id=user.id, provider="other", external_subject="9")
    ...
    linked = await find_oauth_identity(db_session, provider="github", external_subject="42")
...
async def test_unknown_user_returns_none_and_creates_no_rows(db_session) -> None:
    ...
    # Nothing was written.
    identity = await find_oauth_identity(db_session, provider="github", external_subject="42")
    assert identity is None
    user_by_email = await find_user_by_email(db_session, "user@example.com")
...
async def test_github_login_writes_username_on_autolink(db_session) -> None:
    ...
    user = await insert_user(db_session, display_name="Existing")
    await add_email(db_session, user_id=user.id, email="user@example.com", verified=True)
...
async def test_non_github_provider_does_not_touch_github_username(db_session) -> None:
    user = await insert_user(db_session, display_name="U")
    await add_email(db_session, user_id=user.id, email="user@example.com", verified=True)
    await add_oauth_identity(db_session, user_id=user.id, provider="okta", external_subject="42")
    # Stamp a known github_username so we can assert it's NOT overwritten.
    await set_user_github_username(db_session, user_id=user.id, github_username="kept")
