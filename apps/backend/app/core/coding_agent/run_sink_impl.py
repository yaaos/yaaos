...
On each terminal event the sink resolves the coding-agent plugin, runs
`parse_usage(stdout)` + `render_activity(stdout)` to derive the real
`Usage` + `ActivityLog`, and passes them to `finalize_run` which writes
both the run row and the activity blob in the caller's transaction.
"""
...
# The only coding-agent plugin shipped today; the resolver hardcodes it
# until a multi-plugin selector lands.
...
class CodingAgentRunSinkImpl:
    ...
    async def handle_terminal_event(
        ...
        plugin = get_plugin(_CODING_AGENT_PLUGIN_ID)
